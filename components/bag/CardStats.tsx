...
export function CardStats({
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            <button
                                onClick={hasPending ? () => setPendingOpen(v => !v) : handleRefreshMoves}
                                disabled={refreshLoading}
                                title={hasPending ? 'View available moves' : 'Update this card\'s moves from the move library'}
                                style={{
                                    display: 'flex', alignItems: 'center', gap: 3,
                                    fontSize: '0.48rem', fontWeight: 700, letterSpacing: '0.04em',
                                    padding: '2px 7px', borderRadius: 5,
                                    background: hasPending
                                        ? (pendingOpen ? 'rgba(239,68,68,0.15)' : 'rgba(239,68,68,0.08)')
                                        : (refreshLoading ? 'rgba(255,255,255,0.03)' : 'rgba(96,165,250,0.1)'),
                                    border: `1px solid ${hasPending ? 'rgba(239,68,68,0.4)' : (refreshLoading ? 'rgba(255,255,255,0.06)' : 'rgba(96,165,250,0.35)')}`,
                                    color: hasPending ? '#fca5a5' : (refreshLoading ? '#4b5563' : '#60a5fa'),
                                ...
                                return (
                                    <div key={moveIdx} style={{ marginBottom: 8, padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.25)', background: 'rgba(239,68,68,0.04)' }}>
                                        ...
                                        {learnSlot?.moveIdx === moveIdx ? (
                                            <div>
                                                <div style={{ fontSize: '0.5rem', color: '#9ca3af', marginBottom: 5 }}>Replace which move? <span style={{ color: '#f59e0b' }}>(costs 100 coins)</span></div>
    ...
    return (
        <div className="flex flex-col">
            ...
            {/* close row */}
            <div className="flex items-center justify-between px-3 pt-1.5 pb-1" style={{ borderBottom: `1px solid ${borderColor}` }}>
                <span className="text-gray-700 uppercase tracking-widest" style={{ fontSize: '0.5rem' }}>card info</span>
                <div className="flex items-center gap-2">
                    <button onClick={() => setCleanView(v => !v)} title={cleanView ? 'Show details' : 'Clean view'} className="transition-colors" style={{ fontSize: '0.72rem', color: cleanView ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.3)' }}>
                        {cleanView ? '◎' : '○'}
                    </button>
                    <button onClick={onClose} className="text-gray-700 hover:text-white transition-colors" style={{ fontSize: '0.9rem' }}>✕</button>
                </div>
            </div>

            {/* card image */}
            <div style={{ padding: '6px 12px 6px', borderBottom: `1px solid ${borderColor}` }}>
                <div style={{ maxWidth: 190, margin: '0 auto', position: 'relative' }}>
                    <img src={cardImgSrc(uc)} alt={uc.cards.name} className={rainbow ? 'glow-rainbow' : ''} style={{
                        width: '100%', height: 'auto', display: 'block',
                        borderRadius: 8,
                        boxShadow: rainbow ? undefined : rarityGlowShadow(rarity, 'sm'),
                        filter: condFilter, transform: centerSkew,
                    }} />
                    {uc.grade != null && (
                        <span style={{
                            position: 'absolute', bottom: 6, right: 6,
                            fontSize: '0.52rem', fontWeight: 800, fontFamily: 'monospace',
                            background: '#bf1e2e', color: '#fff',
                            borderRadius: 3, padding: '1px 5px', lineHeight: 1,
                        }}>PSA {uc.grade}</span>
                    )}
                </div>
                ...
                <div style={{ marginTop: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2 }}>
                        {uc.is_hot && <span style={{ fontSize: '0.5rem', color: '#fb923c' }}>🔥</span>}
                        <span className={`font-bold uppercase tracking-widest ${rarityClassName(rarity)}`} style={{ fontSize: '0.48rem', ...rarityTextStyle(rarity) }}>{rarity}</span>
                    </div>
            ...
            <div style={{ display: 'flex', gap: 3, margin: '6px 10px', background: 'rgba(255,255,255,0.04)', borderRadius: 7, padding: 3 }}>
                {(['overview', 'pokemon'] as const).map(tab => (
                    <button key={tab} onClick={() => setDetailTab(tab)} style={{
                        flex: 1, padding: '3px 0', borderRadius: 5, fontSize: '0.55rem',
                        fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase',
                        border: 'none', cursor: 'pointer', position: 'relative',
                        background: detailTab === tab ? 'rgba(255,255,255,0.1)' : 'transparent',
                        color: detailTab === tab ? '#e2e8f0' : '#6b7280', transition: 'all 150ms',
                    }}>
                        {tab === 'pokemon' ? 'stats' : tab}
                        {tab === 'pokemon' && hasPending && (
                            <span style={{ position: 'absolute', top: 2, right: 6, width: 6, height: 6, borderRadius: '50%', background: '#f59e0b', boxShadow: '0 0 5px rgba(245,158,11,0.8)' }} />
                        )}
                    </button>
                ))}
            </div>
            ...
            <div style={{ padding: '0 10px' }}>
                ...
                <div style={{ borderTop: `1px solid ${borderColor}` }}>
                    {stats.filter(s => !['MARKET', 'LEVEL'].includes(s.label)).map(({ label, value, color, isLevel }) => (
                        <div key={label} className="flex justify-between items-center" style={{ borderBottom: `1px solid ${borderColor}`, padding: '3px 0' }}>
                            <span className="font-semibold uppercase tracking-widest text-gray-600" style={{ fontSize: '0.48rem' }}>{label}</span>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                {isLevel && evolutionCards.length > 0 && (
                                    <div style={{ position: 'relative' }}>
                                        <button onMouseEnter={() => setShowEvolutionTooltip(true)} onMouseLeave={() => setShowEvolutionTooltip(false)} onClick={() => setShowEvolutionCutscene(true)}
                                            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 14, height: 14, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, animation: 'evolutionPulse 2s ease-in-out infinite' }}>
                                            <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
                                                <circle cx="8" cy="8" r="7" stroke="#34d399" strokeWidth="1.5" style={{ filter: 'drop-shadow(0 0 3px #34d39988)' }} />
                                                <path d="M8 11V5M5.5 7.5L8 5l2.5 2.5" stroke="#34d399" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                            </svg>
                                        </button>
                                        {showEvolutionTooltip && (
                                            <div style={{ position: 'absolute', bottom: '130%', right: 0, background: '#111', border: '1px solid #333', borderRadius: 6, padding: '3px 7px', fontSize: '0.55rem', color: '#94a3b8', whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 50 }}>whats this..?</div>
                                        )}
                                    </div>
                                )}
                                <span className="font-mono font-bold" style={{ fontSize: '0.65rem', color }}>{value}</span>
                            </div>
                        </div>
                    ))}
                    {/* xp bar */}
                    <div style={{ padding: '4px 0' }}>
                        <div className="flex justify-between mb-0.5">
                            ...
                            <span className="font-mono" style={{ fontSize: '0.55rem', color: '#4ade80' }}>{uc.card_xp} / {xpNeeded}</span>
                        </div>
                        <div className="w-full rounded-full overflow-hidden" style={{ height: 3, background: 'rgba(255,255,255,0.05)' }}>
                            <div className="h-full rounded-full" style={{ width: `${xpPct}%`, background: barBg }} />
                        </div>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
                    <button onClick={onToggleFavorite} className="rounded-lg font-semibold transition-all active:scale-95" style={{
                        flex: 1, padding: '5px 0', fontSize: '0.55rem', cursor: 'pointer',
                ...
                {detailTab === 'pokemon' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        ...
                        <div>
                            ...
                            {natureObj ? (
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    <span style={{ fontSize: '0.78rem', fontWeight: 800, color: natureTierColor, textShadow: `0 0 8px ${natureTierColor}55` }}>{natureObj.name}</span>
                                    <span style={{ fontSize: '0.52rem', color: '#6b7280', background: 'rgba(255,255,255,0.05)', border: `1px solid ${natureTierColor}33`, borderRadius: 4, padding: '2px 6px' }}>{natureObj.effect}</span>
                                </div>
                            ) : (
                                <span style={{ fontSize: '0.62rem', color: '#4b5563', fontStyle: 'italic' }}>No nature</span>
                            )}
                        ...
                        <div>
                            ...
                            {combatStats.some(s => s.val != null) ? (
                                ...
                                <span style={{ fontSize: '0.62rem', color: '#4b5563', fontStyle: 'italic' }}>Stats not yet rolled.</span>
                            )}
                        </div>
                        {/* moves */}
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                                <div style={{ fontSize: '0.5rem', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Moves</div>
                                <div style={{ position: 'relative' }}>
                                    <button onClick={hasPending ? () => setPendingOpen(v => !v) : handleRefreshMoves} disabled={refreshLoading}
                                        style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: '0.45rem', fontWeight: 700, letterSpacing: '0.04em', padding: '2px 6px', borderRadius: 4, background: hasPending ? (pendingOpen ? 'rgba(239,68,68,0.15)' : 'rgba(239,68,68,0.08)') : 'rgba(96,165,250,0.08)', border: `1px solid ${hasPending ? 'rgba(239,68,68,0.4)' : 'rgba(96,165,250,0.3)'}`, color: hasPending ? '#fca5a5' : '#60a5fa', cursor: refreshLoading ? 'not-allowed' : 'pointer' }}>
                                        {refreshLoading ? '...' : '↻ Update Moves'}
                                    </button>
                                    {hasPending && (
                                        <span style={{ position: 'absolute', top: -3, right: -3, width: 7, height: 7, borderRadius: '50%', background: '#ef4444', pointerEvents: 'none', boxShadow: '0 0 5px rgba(239,68,68,0.8)', animation: 'pendingPulse 1.5s ease-in-out infinite' }} />
                                    )}
                                </div>
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                {Array.from({ length: 4 }).map((_, i) => {
                                    ...
                                    if (mv) {
                                        const typeColor = TYPE_COLOR[mv.type] ?? '#94a3b8'
                                        return (
                                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 7px', borderRadius: 7, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                                                <span style={{ fontSize: '0.45rem', background: typeColor + '33', color: typeColor, border: `1px solid ${typeColor}44`, borderRadius: 4, padding: '1px 4px', flexShrink: 0, textTransform: 'uppercase' }}>{mv.type}</span>
                                                <span style={{ flex: 1, fontSize: '0.6rem', fontWeight: 600, color: '#e2e8f0' }}>{mv.displayName}</span>
                                                {mv.power && <span style={{ fontSize: '0.5rem', fontFamily: 'monospace', color: '#f87171', flexShrink: 0 }}>{mv.power}</span>}
                                            </div>
                                        )
                                    }
                                    return (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', padding: '4px 7px', borderRadius: 7, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>
                                            <span style={{ fontSize: '0.55rem', color: '#374151', fontStyle: 'italic' }}>— empty slot —</span>
                                        </div>
                                    )
                                })}
                            </div>
                            {hasPending && pendingOpen && (() => {
                                ...
                                <div style={{ marginTop: 8 }}>
                                    {(uc.pending_moves ?? []).map((mv, moveIdx) => {
                                        const typeColor = TYPE_COLOR[mv.type] ?? '#94a3b8'
                                        return (
                                            <div key={moveIdx} style={{ marginBottom: 8, padding: '7px 9px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.25)', background: 'rgba(239,68,68,0.04)' }}>
                                                ...
                                                {learnSlot?.moveIdx === moveIdx ? (
                                                    <div>
                                                        <div style={{ fontSize: '0.48rem', color: '#9ca3af', marginBottom: 4 }}>Replace which move? <span style={{ color: '#f59e0b' }}>(100 coins)</span></div>
                                                        <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                                                            {(uc.moves ?? []).map((existing, slotIdx) => (
                                                                <button key={slotIdx} disabled={learnLoading} onClick={() => handleLearnMove(moveIdx, slotIdx)} style={{ fontSize: '0.48rem', padding: '2px 6px', borderRadius: 4, border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.1)', color: '#fca5a5', cursor: 'pointer' }}>{existing.displayName}</button>
                                                            ))}
                                                            <button onClick={() => setLearnSlot(null)} style={{ fontSize: '0.48rem', padding: '2px 6px', borderRadius: 4, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#6b7280', cursor: 'pointer' }}>Cancel</button>
                                                        </div>
                                                    </div>
                                                ) : (
