import { weightedCondition } from './rarityConfig'
...
export function generateAttributes(rarity: string): {
    ...
    const { base, variance } = RARITY_CONFIG[rarity] ?? {
        base: 6.0,
        variance: 2.0,
    }
    const clamp = (v: number) =>
        Math.round(Math.min(10, Math.max(1, v)) * 10) / 10
...
export function calculateGrade(
    ...
    const base = weightedCondition(attrs)
...
export function centeringSkew(
    attrCentering: number | null | undefined,
    cardId?: string,
): string | undefined {
