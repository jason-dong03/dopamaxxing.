...
// Type-appropriate fallback names for dead moves (0 damage, no effect)
const FALLBACK_MOVE_NAME: Record<string, string> = {
    normal: 'Tackle', fire: 'Ember', water: 'Water Gun', grass: 'Vine Whip',
    electric: 'Thunder Shock', ice: 'Powder Snow', fighting: 'Karate Chop',
    poison: 'Poison Sting', ground: 'Mud Slap', flying: 'Gust',
    psychic: 'Confusion', bug: 'Bug Bite', rock: 'Rock Throw',
    ghost: 'Shadow Sneak', dragon: 'Dragon Rage', dark: 'Bite',
...
function hasEffect(attack: Attack): boolean {
    return attack.damage > 0
        ...
        || !!attack.statusInflict
        || !!attack.selfStatusInflict
        || (attack.selfBoosts?.length ?? 0) > 0
        || (attack.enemyDrops?.length ?? 0) > 0
}
