...
type BattleView = 'menu' | 'story' | 'daily' | 'arena'
...
export default function BattlesPage() {
    ...
    // Story mode is N's storyline only
    const battleTrainerId: TrainerId = 'n'
    ...
    const trainer = TRAINER_INFO['n']
    const trainerPool = getTrainerPool('n', userLevel ?? undefined)
    ...
    return (
        ...
        >
            ...
            {/* ── Banner menu (Story / Daily / Arena) ── */}
            {view === 'menu' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20 }}>
                    {([
                        { id: 'story',  label: 'Story',  desc: "N's storyline",          color: '#4ade80', emoji: '⚔️',  glow: 'rgba(74,222,128,0.18)' },
                        { id: 'daily',  label: 'Daily',  desc: 'Daily challenge',         color: '#fb923c', emoji: '⭐', glow: 'rgba(251,146,60,0.18)' },
                        { id: 'arena',  label: 'Arena',  desc: 'Battle other trainers',   color: '#a78bfa', emoji: '🏆', glow: 'rgba(167,139,250,0.18)' },
                    ] as const).map((b) => (
                        <button
                            key={b.id}
                            ...
                            style={{
                                ...
                                padding: '20px 22px',
                                borderRadius: 16,
                                background: `linear-gradient(135deg, ${b.color}10 0%, rgba(255,255,255,0.02) 60%)`,
                            ...
                            onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.borderColor = `${b.color}88` }}
                            onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = `${b.color}44` }}
                        >
                            <div style={{ fontSize: '2rem', lineHeight: 1, flexShrink: 0 }}>{b.emoji}</div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: '0.7rem', color: '#9ca3af' }}>{b.desc}</div>
                            </div>
                            <div style={{ fontSize: '1.1rem', color: b.color, opacity: 0.7 }}>›</div>
                        </button>
                    ))}
                </div>
            )}

            {/* ── Coming soon view (Daily / Arena) ── */}
            {(view === 'daily' || view === 'arena') && (() => {
                const meta = view === 'daily'
                    ? { label: 'Daily', emoji: '⭐', color: '#fb923c' }
                    : { label: 'Arena', emoji: '🏆', color: '#a78bfa' }
                return (
                    <div style={{
                        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                        gap: 18, padding: '64px 24px',
                        background: 'rgba(255,255,255,0.02)',
                        border: '1px solid rgba(255,255,255,0.05)',
                        borderRadius: 16,
                        marginBottom: 20,
                    }}>
                        <div style={{ fontSize: '3.5rem', lineHeight: 1 }}>{meta.emoji}</div>
                        <div style={{ fontSize: '1.4rem', fontWeight: 800, color: meta.color, letterSpacing: '0.02em' }}>
                            {meta.label}
                        </div>
            ...
            {battleOpen && (
                <BattleScreen
                    ...
                    skipToCardSelect={
                        battleTrainerId !== 'n' || pickedLineupIds.length > 0
                    }
                    preSelectedIds={
